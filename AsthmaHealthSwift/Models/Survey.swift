...
private extension Survey {
    ...
    static func textQuestion(withTitle title: String, surveyId sId: String, questionId qId: String, choiceInfo: [TextQuestionChoice]) -> ORKQuestionStep  {
        let choices = choiceInfo.map { choice in
            ...
            ORKTextChoice(text: choice.text, detailText: nil, value: "ACM\(qId)\(choice.keyword)Choice" as NSString, exclusive: choice.exclusive)
        }
...
private extension Survey.About {
    ...
    static var Task: ORKOrderedTask {
        ...
        let notSmokePredicate = ORKResultPredicate.predicateForChoiceQuestionResult(with: smokingStatus, expectedAnswerValues: ["ACMSmokingNeverChoice" as NSString])
