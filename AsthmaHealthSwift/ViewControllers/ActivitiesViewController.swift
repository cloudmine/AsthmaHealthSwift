...
extension ActivitiesViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        guard let activityCell = cell as? ActivityCell , (indexPath as NSIndexPath).row <= surveys.count else {
            return cell
        }
...
extension ActivitiesViewController {
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        guard (indexPath as NSIndexPath).row <= surveys.count,
            let task = Survey.task(forInfo: surveys[(indexPath as NSIndexPath).row]) else {
            return
        }
...
extension ActivitiesViewController: ORKTaskViewControllerDelegate {
    ...
    func taskViewController(_ taskViewController: ORKTaskViewController, didFinishWith reason: ORKTaskViewControllerFinishReason, error: Error?) {
        ...
        guard nil == error else {
            alert(localizedMessage: NSLocalizedString( "Error completing survey", comment: ""), inViewController: self, withError: error as NSError?)
            return
        }
